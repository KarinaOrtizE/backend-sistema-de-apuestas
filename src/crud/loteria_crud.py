...
class LoteriaCRUD:
    ...
    def crear_loteria(
        self, numero_jugado: str, costo_entrada: float, recompensa: float
    ) -> Loteria:
        ...
        self.db.add(loteria)
        self.db.commit()
        self.db.refresh(loteria)

        return loteria

    def obtener_loteria(self, id_loteria: UUID) -> Optional[Loteria]:
        return self.db.query(Loteria).filter(Loteria.id_loteria == id_loteria).first()

    def obtener_loterias(self, skip: int = 0, limit: int = 100) -> List[Loteria]:
        return self.db.query(Loteria).offset(skip).limit(limit).all()

    def actualizar_loteria(self, id_loteria: UUID, **kwargs) -> Optional[Loteria]:
        loteria = self.obtener_loteria(id_loteria)
    ...
    def eliminar_loteria(self, id_loteria: UUID) -> bool:
        loteria = self.obtener_loteria(id_loteria)

        if loteria:
            sorteo_asociado = self.db.execute(
